...
访问 token 有效期为 15 分钟，后端在剩余 30 秒时用刷新 token 轮换，用户无感。BIFF 会话是 **1 天滑动窗口**：每次成功请求都把 D1 会话与 cookie 一起往后推，连续使用不会掉线，静置超过 1 天才需要重新登录（2026-09-14 由固定 7 天改来）。刷新 token 在后端轮换；并发刷新使用 D1 锁，并保留 30 秒的刷新响应重试窗口。登录时拿不到刷新 token 会直接判为登录失败，不再建一个 15 分钟后必然失效的会话。上游判 token 无效时只记日志、不删会话，避免一次抖动把人永久踢出。退出 BIFF 删除其会话并撤销刷新 token。切换账号会重新进入 IFFDAY 登录流程。

## 本地数据与同步

登录后，尚未导入过的账号会自动合并非空本机数据。合并前保留本机副本。选片、场次、备注、已保存方案和现有 `biff.*` 偏好都参与同步，公开的影片资料仍从静态文件读取。

导入完成状态存储在 D1 的 `account_import` 表，以用户 ID 唯一标识，跨浏览器、设备和影展届次生效。只有片单写入与标记创建在同一 D1 batch 中成功提交后，账号才算已导入。空 localStorage、空数组/对象、null 和空字符串不会占用导入机会；失败与 revision 冲突也不会。已导入账号在新设备上只加载云端数据，访客副本仍保留本机。有实际冲突时，用户确认保留的版本后再提交。
