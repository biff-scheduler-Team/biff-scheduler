...
- **★ 所有改动必须先读 [`docs/DEVELOPMENT-STANDARDS.md`](DEVELOPMENT-STANDARDS.md)(规范 v1,2026-09-13 定稿)——
...
- **★「影片库 · 我的选片」= 左侧「挤压式抽屉」**(2026-09-10 二次改,`PLAN-20260910184745`;
  ...
  **2026-09-10 起「我的行程」从 `#agenda-wrap` 搬入抽屉第三个 tab**,见 `PLAN-20260910190916`)
  ...
  · **★ 新版桌面工作台上限(2026-09-14,PLAN-20260914003600)**:React `#workspace` `max-width: 1600px` 居中；
    桌面 ≥1100 我的观影与排片为 **1:3 挤压分栏**（窄屏仍浮层）。Legacy 近全宽口径不变。
...
- **★「只有一场」的影片:选定 = 排定(2026-09-16,`PLAN-20260916004024`,含修订 1)**:
  判据唯一来源 = `model.ts::soleShowIndex()`(按 `filmNodeKey` 分组,排期里**恰好一场**才入表),
  由 `store.tsx::hydrateStorage()` 注入 `state.ts::registerSoleShows()` ——
  **未注入 = 旧行为**,所以纯逻辑单测与 legacy 侧不会被误伤,也**绝不会把多场片当单场片**。
  三处口径都走它:
  ① **载入补齐** —— 单场片若已在选片清单里却没有场次(旧版本只建了空记录)→ `fillSoleShowPicks()` 排上那一场;
  ② **影片库入口** —— 按钮写「加入行程」,`addPickFilm(key, soleCode)` 直接落场,加完转「已在行程，去查看」;
  ③ **取消** —— 移出单场片的唯一场次 = **连选片记录一起删**(留下空记录会被 ① 补回来,用户看到的是「删不掉」),
    统一走 `components/screening-actions.ts::useScreeningPicker()`,**先 `confirm` 说明**再动手;多场片一律只移出该场。
    · 判据唯一来源 = `screening-actions.ts::removalDropsPick(cat, s)`(2026-09-22 抽出,`PLAN-20260922123138`)——
      拦不拦、以及弹层怎么写这句文案,读的是同一处。
    · 调用方**已经**把这件事说清时(「我的行程 → 日程表」画布上点掉一场的站内确认弹层,
      见 `ScheduleGantt.tsx::AgendaRemoveDialog`),传 `{ soleShowAsked: true }` 跳过这个原生 `confirm` ——
...
- **★ 抢票顺位 = 偏好次序(2026-09-11,`PLAN-20260911223000` 二改)**:`state.ts::rankOf: Map<code, number>`
  ...
  **它不决定分组、也不决定「哪套是方案几」**。
  ...
  · UI 唯一入口 = 「我的行程」冲突组**顺位卡**里卡片头第 1 列的 `⠿` 把手(`AgendaPage.tsx` 的 `RankGroup` /
    `startDrag`,`agenda.ts::attachRankDrag` 是 legacy 那份)。
    **用指针事件而非 HTML5 DnD**(移动端不支持 DnD,而顺位手机上也要能改);拖动中直接改 DOM 顺序,
    松手才 `setRanks` 落盘 + 广播。
    ⚠ **顺位卡自 2026-09-22 起只在「卡片」视图里按日就地展开**(`PLAN-20260922123138`):
    日程表画布上的那份(含「冲突组顺位」侧栏)与「顺位撞车」提示都已下线,用户口径是
    「我的行程不需要显示冲突组顺位这个组件 直接铺满」。顺位机制本身一个字没动。
...
- **★ 三个「出口」各司其职(2026-09-11)**:`.ics`(`ics.ts`)= 喂日历(机器读);分享文案(`share.ts`)= 按日程读的
  「**三行缩进块 + 日期分节**」纯文本(给人看,贴微信);分享图片(`poster.ts` + `poster-panel.ts`)= 同一份行程的
  ...
  **不要再合并成一个「导出」函数** —— 排序口径与信息取舍本就不同。
  ...
  · **★ 分享文案版式 = CODE 行首 + 三行缩进块 + 无 emoji + 无 Markdown**(2026-09-16,`PLAN-20260916002752`):
    一场 = `004  09:20–11:09  英文名` / `                  中文名` / `                  BCC 1 · 主选 · 映后`,
    续行缩进到**英文名的起始列**(按实际首行前缀宽度算 —— 跨午夜时间串多出「次日 」,写死空格数会错位)。
    **CODE 提到行首**是因为分享的第二个用途是**抢票分工** —— 朋友要按放映代码去官网找场次 / 群里报号,
    行首才对齐成列;挂在影院行尾时它随影院名长短左右漂移。**影院行不再重复印 CODE**。
    **不用 emoji**:旧版的 🎬 / 📅 / 📍 / 📝 全去掉(各端渲染宽度不一,缩进对不齐)。
    **不用 Markdown**(微信不渲染,`**` / `|` 会原样印出来)。
    状态列 = 影院短名 · GV 标记(` · ` 连接,空位自动省略)——
    ⚠ 原先还有一个顺位标记(`主选` / `备选②`),已于 2026-09-22 去掉(见下面那条)。
    GV 标记 = `映后`(含谈) / `仅正片`(弃谈) / `GV`(有 GV 但没配谈段时长)——
    ⚠ 它被分享文案 / 分享图片 chip**两处共用**(`poster.ts::buildPosterModel` 的 `gv` 字段也走它),改文案会一起变 ——
    原先还有第三处「方案『查看场次』弹层」,随「已保存方案」整体下线(2026-09-22,`PLAN-20260922105228`)。
    日期分节头 = `【OCT 7 · 周三】`(场次数由概要行的「共 N 场」给出,不再重复印)。
  · **★ 分享不再有「顺位 / 开票批次」两个可选扩展(2026-09-22,`PLAN-20260922102751`)**:
    用户原话「分享的地方去掉顺位和批次了」—— `share.ts::ShareOptions` / `ShareRanking` / `ShareBatching`、
    `rankMark` / `ALT_MARK`、`shareUnits` / `batchSections`、以及 `batch.ts` 整体删除;
    导出弹层那两个勾选框也删了。**分享文案与分享图片现在恒产出一种版式**(CODE 行首 + 三行缩进块 + 日期分节)。
    ⚠ 被删掉的**不是顺位本身**:选片顺位(`biff.ranks.v1` + 行程页冲突组拖拽 + `plans.rankOf`)照旧,
    只是不再印进分享内容 —— 理由:`ranking` / `batching` 让一份「对外成品」多出两个与应用内状态耦合的开关,
    而它们服务的是**抢票当天**的分工,那个场景已随「抢票」页一起下线。
    历史口径(备选按自己的批次归节、`349 的备选②` 这类措辞)只在 git 历史与 `PLAN-20260916011511` 里。
  · **★ 「已保存方案」已整体下线(2026-09-22,`PLAN-20260922105228`)**:用户原话「我觉得现在方案没有什么意义了
    去掉也可以」→ 选**整体下线**。`state.ts` 的 `SavedPlan` / `savedPlans` / `savePlan` / `deletePlan` /
    `loadSavedPlans` / `persistSavedPlans` / `sameCodeSet` / `nextPlanName` / `planById`(最后一个当时已是死代码)、
    `components/PlanShowsDialog.tsx`、`agenda-model.ts::describeSavedPlan`、行程页的「保存当前方案」按钮与
    「已保存方案」区块、`sync-data.ts` 的 `plan:` 专列、`workspaceCounts().plans` 全部删除。
    ⚠ **`biff.savedplans.v1` 这个键留着不删、不迁移**(同 `biff.discussionprivacy.v1`):不再读、不再写 ——
    这样「加载旧数据 → 原样回写」的字节级断言(`compatibility.spec.ts`)与 `backup.ts` 的 `biff.` 前缀快照
    都照常通过;哪天想重启这功能,键里的旧数据还在。**别顺手加一条「删旧键」的迁移。**
    ⚠ **顺位不是方案的一部分**:`biff.ranks.v1`、冲突组拖拽、`plans.ts::rankOf` 全部保留;
    `agenda-model.ts::topPlanCodes` 也保留(导出范围的「当前行程」还在用它)—— 它的名字带 plan 是历史包袱,
    **别照它推出「还有方案」**。
    ⚠ `apps/web/legacy/**` 里那份实现**不动**(只读回退件,见 §5);`e2e/react/fixtures/legacy-state.ts.txt` 同理。
  · **★ 导出范围只剩「当前行程」一项(2026-09-22,`PLAN-20260922105228`)**:「已保存方案」整体下线后,
    原先那个 `导出范围` 下拉(`PLAN-20260916135942` 起的第一项 = 当前行程)被**整条删除**,换成一**行静态说明**
    `导出范围：当前行程（29 场，OCT 7–OCT 10）`(口径仍走 `ExportDialog.tsx::codesOutline`)——
    只剩一个选项的下拉是个点了没变化的死控件。范围真值 = `agenda-model.ts::topPlanCodes(plans)`
    (当前行程「每组第一顺位 + 共同场次」)。`.ics` 与分享文案 / 分享图片同用一个 `rows`,三者集合一致。
    ⚠ 出图作废判据随之从「所选方案的 id」换成「**场次集合签名**」(`ExportDialog.tsx::scopeSignature`):
    出图是异步的,期间行程可能被本页或**别的标签页**改掉,集合一变那张图就作废 —— 否则用户会拿到一张
    与当前行程对不上的图。旧判据的两半(「所选方案被别的标签页删掉 → 回落最后一个」「都没有 → 回到当前行程」)
    随方案一起删除:不再存在可被外部删除的范围对象。
    ⚠ 2026-09-22 起**不再有「范围 vs 顺位 / 批次」的耦合问题** —— 顺位与批次已从分享里下线
    (`PLAN-20260922102751`),原先那条「场次取所选范围、顺位取当前行程,混用会自相矛盾」的警告随之失效。
  · **★ 「场次 → 开票批次」的判据已退役(2026-09-22,`PLAN-20260922102751`)**:唯一来源
    `batch.ts::ticketBatchOf` 已随「抢票」页与分享批次分节一起删除。**批次现在只剩「开票时间」这一层**,
    数据来自 `apps/web/public/festival-extras.json::ticketing.batches`(经 `extras.ts::ticketOpens` /
    `batchHeading` / `nextTicketOpen` / `countdownText`),出口是顶栏「距第 N 批开票」倒计时、购票信息弹层、
    开票提醒 `.ics` —— 它们**从不依赖** `batch.ts`。历史判据(露天按 `venue_id ∈ {bt}`、
    Community BIFF 按 `^9\d\d$`、`actors_house` 活动等,见 `PLAN-20260915234414`)只在 git 历史里,别照抄。
  · **★ 「抢票」页 —— 已整体下线(2026-09-22,`PLAN-20260922102751`,用户「抢票入口也去掉吧」)**:
    导航项、`/rush` 路由、`fullPage` 正则里的 `rush`、`pages/RushPage.tsx`、`pages/rush.css`、
    `e2e/react/rush.spec.ts`(其中两条分享用例迁为 `e2e/react/share-export.spec.ts`)全部删除。
  ...
  · **★ 分享图片的版式(2026-09-22 起)**:海报行首 = `CODE  时间`(与文案同序:抢票时按 CODE 找场次),
    影院行不再重复印 CODE;按日期分节 + 每天「N 场」计数 —— 与分享文案同源(`orderedPickRows` / `shareSummary`)。
    ⚠ **顺位标记、备选行、开票批次节头已于 2026-09-22 随分享选项一起删除**(`PLAN-20260922102751`):
    `PosterRow` 不再有 `rank` / `alts` / `altOf`,`PosterModel` 也不再分节(`sections` 已删,直接用按天的 `days`
    —— 原先它是「为批次分节而生」的中间层,批次没了它就是恒单元素的死壳)。
    ⚠ **几何:`poster.ts::rowHeight()` 是行高的唯一来源**(`posterHeight()` 与 `drawPoster()` 都读它)——
      各算一份会出现「按 3000 高建画布、实际画了 3200」,**多出来的内容被裁掉且不报错**(最难查的一类)。
  ...
  `**` / `|`)、**不用 emoji**(状态位走文字,`↳` 是箭头符号不算)、每行尽量短(片名 / 影院各自单独一行)。
  复制一律走 `clipboard.ts::copyText`(带 `execCommand` 降级)。
...
- **★ 形状与灰阶:**两档 token 定了「同一屏里别再出现第三种圆角 / 第二种灰」(2026-09-22,`PLAN-20260922105228`):
  · **圆角只有三档**(`style.css` 的 `:root`):`--radius-chip`(小件:图例章 / 场次格 / 折叠按钮)、
    `--radius-card`(面板 / 卡片 / 日期卡 / 说明弹层)、`--radius-pill`(药丸:CODE 徽章 / 计数章 / 概览 tag)。
    新写组件**一律读 token**,别再写 `border-radius: 4px|6px|8px|12px` 字面量 ——
    此前同一屏里同时出现过 4 / 6 / 12 / 999 四种值,肉眼可辨。
    ⚠ **例外(不是漏改):卡片内部的缩略图**(剧照 / 海报 / 成员头像)用 3px —— 那是**内层圆角**
    (外圆角 6 − 内边距 3 = 3),不参与这一档 token。
  · **说明文字的灰只有两档**:`.muted`(旁注:图例 / 计数 / 旁白)与 `.muted-strong`(要**读进去**的操作指引:
    顺位卡怎么拖 / 空态为什么空 / 概览 tag)。
    ⚠ **不要全局加深 `--muted`**:它同时是分隔线 / 图例 / 大量非说明文字的颜色,加深会连带改掉
    排片表与影片库的观感(用户第 5 条要的是可读性,不是整体换灰阶)。
...
- **★ 票据明细 = 手填的座位表,「座位行数就是票数」(2026-09-24,`PLAN-20260924141442` 修订 3 起)**:
  ...
  · `account` 存的是账号**名**标签(如 `foxmail`),**不是凭据** —— 本结构落在 `biff.` 前缀下会随片单上云,
  ...
  · 为什么另起一只键:三态那份回答「抢到了没有」(驱动「实际行程」筛选与同场人数),这份回答「手上有几张、坐哪」
    ——塞进同一个值里会同时踩「`biff.*` key 只增不改」与「旧版 `normalizeTicketRecord` 静默丢字段」两条。
  · **票数一律由行数派生**(`ticket-info.ts::ticketCountOf`),**没有**独立的数字字段 ——
    用户口径是「加一张座位 = 多一张票」;两个来源必然打架,所以只留一个。
    推论:`seats` 归一化**绝不许收掉空行**(`["F12",""]` 是两张票,收掉尾部空项票数就变 1)。
  · **「共 N 张票」的唯一口径** = `ticket-info.ts::totalTicketCount`(「已抢到」∪「有明细」,没明细的按 1 张)——
    它与概览里那条按**场次**数的「实际 N 场」刻意并存(一场可能买 2 张)。
  · **场次移出行程 → 票务明细一并删除**,这是**用户 2026-09-24 明确选的**「票务与行程绑定」的忠实语义
    (没票的场次留着票务信息也没有意义)。⚠ 代价写明:座位号 / 预约号 / 姓名是**手录、算不回来**的,
    移出行程就一起没;要恢复只能靠「导出数据备份」里的那份(备份按 `biff.` 前缀全量快照,含票务明细)。
    **别在没问过用户的情况下把它改成「留着」** —— 那会让「票数」与「行程场次数」重新出现两套口径。
  · 编辑入口 = 日程表格子**右键** + 格子上常驻的票按钮(触摸设备没有右键,单入口等于手机上功能不存在)。
  · **★ 「不划位」(自由入座)没有独立字段(2026-09-24,同 PLAN 修订 4)**:落库形态就是**全空的座位行**
    (`["",""]` = 两张票、没有座位号),「不划位」是弹层从行内容**推断**出来的**显示档位**
    (`ticket-info.ts::allSeatsBlank`),**只活在界面上、不落库**。
    · 用户口径(2026-09-24):「有不划位的 需要考虑这种情况」—— 划位场要逐行填座位号,不划位场没有座位号可填,
      在弹层里逼用户面对一排空的「座位号，如 F12」是说不通的。
    · **为什么加字段被否**:加 `unreserved` 就得多起一只 `biff.ticketinfo.v3` + 一条迁移链
      (旧版 `normalizeTicketInfo` 会静默丢新字段,这是 `biff.*` key 只增不改的由来);用户 2026-09-24 明确选了不加。
    · **已知代价(如实记下,别当它不存在)**:数据上分不清「不划位」与「划位但座位号还没抄」。
      两者**票数完全相同**,故 tooltip 只陈述事实(「N 张，座位未填」),不去猜是哪一种
      —— 旧版那句只报「N 张」会让人怀疑界面丢了东西。
    · **清空座位号只发生在保存时**(`TicketEditDialog::save` 的 `finalSeats`),不在切档位时:
      切档位只是换个显示面,来回切一下不该丢掉刚抄下来的座位号(切回去还在,保存才算数)。
      但保存时必须清 —— 留着旧值会让记录下次打开被判回「划位」档(`allSeatsBlank` 为假),
      明明标了不划位、重开却变成划位。张数不受影响(它由行数派生)。
    · **初始档位** = `ticket-info.ts::defaultUnreserved`:露天场(`venue_id === "bt"`)**且**非开闭幕
      (`tags` 不含 `opening` / `closing` → `001` / `002` 也是 `bt`,但那是典礼场,固定座席)。
      ⚠ 这是**常识默认,不是数据**:`schedule.json` / `venues.json` / `festival-extras.json::ticketing`
      三处都查过,没有任何「划位 / 不划位」字段,理不出逐场判据 —— 所以它只覆盖唯一有据可依的那类场景,
      且**永不写回存储**;未知场馆一律默认「划位」(猜错成自由入座会让人以为座位号丢了)。
  · 改结构时按 AGENTS §5 走「新 key + 一次性迁移 + 删旧 key」:`v1`(`{count, seats, accountId}`)由
    `ticket-info.ts::migrateTicketInfoV1` 折算成「补齐到旧 count 那么长的座位行」,迁完即删旧键。
- **★ 票务 JSON 导入 = 只写票务的增量通道(2026-09-24,`PLAN-20260924141442` 修订 5)**:
  入口在「更多 → 导出与分享 → 导入备份 / 日历 / 票务」(`ExportDialog.tsx::ImportData`),
  解析在 `apps/web/src/ticket-import.ts`(纯逻辑、node 可测)。
  · **为什么不复用备份通道**:`backup.ts::restore` 是**整体替换**(先清空本机全部 `biff.` 键再写入)。
    用户已经在用这个工具(有选片 / 顺位 / 设置),拿它导票会把这些全洗掉 —— 所以票务导入只写
    `biff.ticketinfo.v3` + `biff.tickets.v1`(按需)+ 行程,是**合并**语义。
  · **★ 顺序红线:先并行行程,再写明细**(`ExportDialog::applyTickets` → `state.ts::applyTicketImport`)。
    `rebuildIndex()` 会把不在行程里的票务明细当脏数据 prune 掉 —— 顺序反了,刚导进去的明细会在
    下一次 rebuild 时**凭空消失**。这是本功能最容易写错的一处,E2E 用「import 完后 reload 再看」
    钉住(空行程起点)。
  · 一个框收三种内容(**备份 / .ics / 票务**),按**顶层形状**分流
    (`ticket-import.ts::looksLikeTicketImport`),各报各的错 —— 把票务 JSON 当备份解析,报出来的会是
    「没找到任何 `biff.*` 数据」,用户看不懂。分流判据只看形状、不解析每条;解析不了就当票务**不成立**,
    交给备份通道报既有的那句「不是有效的 JSON」(既有断言不变)。
  · `seats` 认四种写法:数组 / **数字**(= 这么多张、都不划位)/ 单个字符串 / 省略(= 1 张)。
    **同一个 code 出现两次直接报错**、不静默取后者 —— 「导入成功但少了一半票」那种错很难被发现。
  · 导入连带**标「已抢到」**(只在当前不是 `got` 时写,且保留既有 `via`),于是概览的
    「实际 N 场 / 共 N 张票」立刻对得上。片单里查不到的 code 跳过并在预览里报出条数。
- **★ 票务凭据一律不存(2026-09-24,`PLAN-20260924141442` 修订 3)**:「在设置里存 BIFF 官网账号密码、
  按账号跳订单页」的方案**已整体撤销**,不留任何键、任何字段。
  · 撤销理由(实测,不是推测):`ticket.biff.kr` 会 302 到第三方平台 `biff.maketicket.co.kr`,
    订单页(`Login → My page`)在**另一个域**的登录态后面。跨域意味着本站既写不进它的登录框、
    也建不了它的会话 —— 存密码只能换到「手动复制再粘贴」,收益≈0,代价是明文密码躺在本地磁盘上。
    跳转本身不依赖我们存任何东西:用户在那边登录过就能直接进 My page。
  · 因此 `biff.ticketinfo.*` **不许出现任何凭据字段**;上面那条座位表结构里也没有位置放它。
  · 真要帮用户记「票在哪个账号」,写进座位表之外也无处可去 —— 这个需求已明确不做,不要再复活一半。
...
- **★ 红黑榜投票口径(2026-09-16,PLAN-20260916102339)**:一人**一部一票**(值为红 / 黑),**不做加权** ——
  与「想看人数」的 0.75 / 1.0 刻意不同:贴纸是离散的实体隐喻,3 个人贴了红就该显示 3,
  加权会读出 2.25 这种没人看得懂的数。防刷靠贡献表 `(edition, film_key, contributor)` 的唯一约束。
  · 上报 `POST /api/stats/film-votes-ping` 只发**影片 key 与颜色**(1200ms 防抖、**整份替换**,断网后自愈),
    读取 `GET /api/stats/film-votes` 一次拿全部影片的红黑票数;隐私边界同 want-ping(只发 key,不发身份 / 坐标)。
  · ⚠ 登录时**三张贡献表都要清**(`screening-stats-store.ts::clearAnonContributions`):少清一张,
    红黑榜就会多出一票(同一人「匿名 + 登录」各算一次)。
...
- **★ 分享图(2026-09-22,PLAN-20260922164728)**:应用有**两张**对外出图的分享图 ——
  ...
  · **每行是两列:左文字 + 右「贴纸区」**(2026-09-22 用户:「我想在分享页把贴纸也展示出来吧 现在只有数字」,
    后经「为什么不能也沿用两列 然后有一个专门的贴纸绘制区」改为两列):
    数字答「多少」、贴纸区答「长什么样」。版式与页面卡片同源(`.rb-card` = 左文字 + 右画布),别另造第三种表达
    (第一版试过「横贯整行的 32px 贴纸带」,纵坐标被压扁成一涂,且比例条已经说过一次比例 —— 已弃)。
    落点 / 歪斜复用 `spotOf` / `tiltOf`,群点取 `othersOf`(只画别人的)、我那一枚追加在末尾并加亮边
    —— 与卡片画布**同源同序**;票数不打折。
    ⚠ 贴纸区只有 88px 高,坐标映射要**收进区内**(卡片画布 176px 高没有这个问题),否则上下沿会被切平。
    ⚠ 贴纸区宽度(`FIELD_W = 360`)是从文字列让出来的:调大它 = 片名能显示的字数变少(可用宽度 516px)。
  · **长图内容可勾选**(2026-09-22 用户:「分享的时候 选一下需要分享的长图内容吧 有的人不想所有的榜单都分享」):
    「长图内容」勾选组四个节(总数 / 红 / 黑 / 我贴过的),**默认全选**(少给才需要动作);
    勾掉的节连算都不算,渲染顺序按固定档位序;「我贴过的(N 部)」0 部时置灰;
    一节都不勾时下载 / 复制停用,空态文案分「还没选」与「真的没票」两句 —— 共用一句会把用户引到错处。
    ⚠ E2E 里别拿**节名**当判据:图里的大标题「红黑榜」本身就含「黑榜」,要用各节的口径说明
      (「按黑贴纸数」);点 RAC 复选框要点**标签文字**(真 `<input>` 是视觉隐藏的,`uncheck()` 会被挡)。
  · **署名画两处**:头部(届次那一行右对齐)与底部各一处 —— 长图常被截一半转发,底部那行会跟着丢。
    两处共用 `model.credit.by`;E2E 用 `painted.split(署名).toHaveLength(3)` 判「出现两次」。
  · **尺寸提示**:弹层里常驻一行「宽 × 高 px · N× 出图 · PNG 约 M MB」,宽高走 `rbPosterHeight` +
    `posterScale`(与实际出图同一处判断),MB 取 `blob.size`(实测不是估)。
    **尺寸只由行数决定**:`高 = 520 + Σ(128 + 104 × 该节行数)`(520 = 上下红条 + 头部 + 页脚;
    每节 128 = 空档 32 + 节头 96);另外 `1080 × 高 × 4 > 16M`(高 > 3703)时回 1×。
  · ⚠ **超采样与退档只有一处**:`poster-brush.ts::posterScale(w, h)` —— 同时守两条上限:
    画布**单边** 32767(`SCALE_DOWN_H`)与画布**面积** 16M px(`MAX_PIXELS`,iOS Safari 约 16.7M)。
    后者是 2026-09-22 补的:2× 下逻辑高超过 ~3700 就 `toBlob` **静默出空图**(长行程的分享图在
    iPhone 上一直是一张空图)。**宁可 1×,也不能出空图**,别把倍率判断写回各图里。
...
- **★ 红黑榜榜单排序口径(2026-09-22,PLAN-20260922161710)**:三档都走 `redblack.ts::sortByCounts()`
  ...
  不是第四档排序(塞进 `sort` 会与「顺序冻结 / 重新排序」那套机制纠缠)。
  · ⚠ 票数是**异步**到的:首屏那次排序发生在它之前(每部并列 0 → 稳定排序退化成**影片库目录序**),
    界面上却写着「按总数从高到低」—— 所以**票数一结算就自动补排一次**(仅此一次),之后照旧**冻结**
    (2026-09-16 用户:「跳动很频繁」),只有换档位 / 改搜索词 / 点「重新排序」才重排。
  · ⚠ 补排有**门槛**:用户已经滚过 / 点过就跳过 —— 读到一半被整页重排顶走正是当年那个痛点。
    跳过时界面上的「有新贴纸 · 重新排序」就是留给他自己补排的。
  · ⚠ 「有新贴纸」的判据是 `crowdSignature`(票数**内容**签名),不是 `votes` 的对象引用:
    重拉必然拿到新对象,拿引用比会让提示在数字没变时白亮一次(2026-09-22 修)。
- **★ 红黑榜画布的渲染口径(2026-09-22,PLAN-20260922145815)**:**票数几枚就画几枚**,不再有「每卡最多
  ...
  · 「我贴的那一枚」是 DOM 上唯一的贴纸元素(`<button>`,可点、可键盘收回):**单击它收回暂存区**,
    ...
    (`FRESH_MS = 2400`,有界)。闪完之后它**仍比群点多一圈常驻纸白边**(2026-09-23,见下一条),
    「与别人逐字一致」的旧口径只适用于**贴纸本体**(形状 / 尺寸 / 实心度),不适用于这圈辨识边。
    ⚠ 「算拖还是算点」必须靠 `movedRef` 区分:`pointerup` 之后浏览器**还会**补发一次 `click`,
    不区分的话「微调位置」会变成「撤销」(阈值分档:鼠标 4px / 触屏 10px)。
    ⚠ 但**只有指针来的 click 才问 `movedRef`**(判据 `MouseEvent.detail > 0`):键盘 `Enter` /
    空格与 `element.click()` 的 `detail` 恒为 0、不经过 `pointerdown`,一刀切会让「拖一次 →
    再按回车收回」被静默吞掉(2026-09-23 review 修)。
  ...
  · ⚠ 贴纸外观只允许一处实现:`redblack-parity.css` 的 `.rb-dot` 是 DOM 版(我贴的那枚 / 暂存区 / 拖拽浮标),
    ...
    ⚠ **唯一例外 = `.rb-dot` 上那圈常驻纸白边**(2026-09-23):它是「我贴的那一枚」的辨识,
    `sticker-sprite.ts` **不许**跟着加 —— 差异只许单向存在,加了就又拉平了(见下一条)。
...
- **★ 红黑榜拖拽口径(2026-09-23,`PLAN-20260923101147` + `PLAN-20260923103830` + `PLAN-20260923104622`)**:四件事。
  · **张贴区按片独立** —— 一枚贴纸的归属就是它那一部电影的 `key`,别片的画布不是落点。
    `redblack.ts::moveSticker` **不再收 `toKey`**(跨片分支已删,数据层不留「永远不该被调用」的路径);
    出界判定收窄到**本片**画布(`RedBlackPage.tsx::finishRef`):已贴的 → 收回暂存区(沿用 `takeBack`,
    文案只此一处),从暂存区拖出的 → 什么都不做但给一句说明。拖拽高亮也只落**本片**卡片。
    ⚠ 旧口径会把《A》的票**直接改记到《B》头上**,别再放开。
  · **同一时刻只允许一个拖拽手势**(`gestureRef`):监听器是**同步**挂在 `window` 上的(`beginDrag`,不再是
    `useEffect([drag])`),所以旧实现那层「cleanup 先摘掉上一套监听」的保护没有了 —— 两指的 `pointerup`
    会各跑一遍自己的 `onUp`,**一次松手结算两次落点**。现在:第二个指针 `pointerdown` 直接忽略,
    `onMove` / `onUp` 一律按 `pointerId` 过滤,卸载时跑一次收尾(否则监听器永久留在 `window` 上,
    而 `finishRef` 还指着已卸载树的闭包、仍能写 localStorage)。
    ⚠ **另有一条兜底**(2026-09-23,`PLAN-20260923104622`):鼠标在**窗口外**松手时那个 `pointerup`
    可能根本没送到页面,`end()` 就永不执行 —— 于是 `onMove` 里多判一句「**不是触屏**却 `buttons === 0`」
    → **只收尾、不结算**(那个坐标往往是鼠标**重新进入窗口**时的第一帧,不是松手点)。
    没有它的话 `gestureRef` 一直占着,**此后每一次拖拽都在 `beginDrag` 第一行被挡掉**、ghost 还粘在鼠标上。
  · **我贴的那一枚有常驻纸白边**(`.rb-dot` 的 `box-shadow` 第二层 `0 0 0 1.5px rgb(255 255 255 / 88%)`):
    一片同色同尺寸的群点里得先**找得到**自己那枚,「自己贴的贴纸始终能被自己拖动」才成立 ——
    只有「刚贴」那 2.4s 有描边是不够的。⚠ 用**白**不用 `--brand`:品牌红 `#ce1e36` 与红贴纸 `--rb-red`
    `#cf3b52` 几乎同色,红圈在红贴上等于没画。⚠ `sticker-sprite.ts`(群点)**不加**这一层 —— 差异单向。
    「刚贴」那 2.4s 的 `--brand` 3px 描边**保留**,读起来是「在常驻白边之上又亮一档」,不是两套语言。
    ⚠ 触屏:`touch-action: none` **只加在 `.rb-src`(暂存区那 32px 按钮)与 `.rb-dot` 上**,
    `.rb-canvas` **千万不要**加 —— 那一片是卡片右半张,加了手指在那块区域就没法滚页面。
  · **放大弹层与卡片是同一组成**(2026-09-23,`PLAN-20260923104622`):`.rb-zoom-stage` 里画布只画
    `othersOf(counts, mine)`,我那一枚是**压在画布之上**的 `.rb-dot`(加 `.rb-dot--still`:只读变体,
    只改光标与命中,外观全继承)—— 白边因此仍只有一处实现,位置也是它的**真实坐标**。
    ⚠ 改之前这里把 `all` 交给画布:我那一枚被画成 `crowdStickers(filmKey, all)` 的**最后一枚**,
    那是由 id 推导出来的点,既不是它在卡片上的真实落点、也没有那圈白边 —— 而 lede 还写着
    「位置与卡片上一致」(lede 的数字口径不变:仍是全体,即卡片上那个红黑数)。
  · **拖出张贴区要有明显提示**(2026-09-23,`PLAN-20260923110401`):「松手会把它收回暂存区」这件事
    必须在松手**之前**看得出来 —— 出界时三处同时给信号:① 浮标换装 `.rb-ghost--out`
    (半透明 + **去饱和**;不靠换描边色:`--brand` 红在红贴纸上、白环在亮色卡片上都等于没画)
