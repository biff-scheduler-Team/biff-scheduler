...
- **★ 单场影片的「加入我的选片」= 直接落进行程(2026-09-16,`PLAN-20260916004024`)**:
  判据是**全量** `film.shows.length === 1` —— 是则影片库按钮写「加入行程」,点击经
  `state.ts::addPickFilm(key, soleShowCode)` 把那一场直接排进行程(同一条 `commit()` 出口:落盘 / 重建索引 / 广播),
  按钮随后转「已在行程，去查看」;多场片维持原两步(只建空记录,按钮写「加入我的选片」)。
  · 判据**不得**用视图里筛选后的场次 —— 「只有一场」是数据事实,不该随筛选器变化。
  · `legacy/` 不跟进(只读回退件,见 §5)。
