...
- **★ 三个「出口」各司其职(2026-09-11)**:`.ics`(`ics.ts`)= 喂日历(机器读);分享文案(`share.ts`)= 按日程读的
  ...
  **不要再合并成一个「导出」函数** —— 排序口径与信息取舍本就不同。
  ...
  · **★ 分享文案版式 = CODE 行首 + 两行一场**(2026-09-15,`PLAN-20260915234414`):
    场次行 = `004  09:20–11:09  英文名 · 中文名`,第二行 = `📍 影院短名 · GV 标记 · 顺位 N`。
    **CODE 提到行首**是因为分享的第二个用途是**抢票分工** —— 朋友要按放映代码去官网找场次 / 群里报号,
    行首才对齐成列;挂在影院行尾时它随影院名长短左右漂移。**影院行不再重复印 CODE**。
    GV 标记同步缩短(`GV 含映后谈` → `含映后谈`、`GV 仅正片` → `仅正片`、仅 GV 无谈段仍印 `GV`)——
    ⚠ 它被分享文案 / 分享图片 GV chip / 方案「查看场次」弹层**三处共用**,改文案会一起变。
  · **★ 分享文案的两个可选扩展(`share.ts::ShareOptions`,导出弹层的两个勾选框)**:2026-09-15 加。
    · `ranking` → 场次行尾 `· 顺位 N` + 同冲突组其余场次逐行 `↳ 备选 …`;
      **顺位 / 备选一律取「当前行程」的冲突组**(`plans.ts::groupMatesOf` + `plans.rankOf`),
      **不是方案快照** —— 方案每组只留第 1 顺位,拿它印顺位恒等于「顺位 1」,朋友没法按顺位分工。
      备选行**省日期**(冲突按日期隔离 ⇒ 同组必然同日)、**必须印片名**(备选常是另一部片)。
