...
- **★ 分享图(2026-09-22,PLAN-20260922164728)**:应用有**两张**对外出图的分享图 ——
  ...
  · **每行摊一条「贴纸带」**(2026-09-22 用户:「我想在分享页把贴纸也展示出来吧 现在只有数字」):
    数字答「多少」、带子答「长什么样」。落点 / 歪斜复用 `spotOf` / `tiltOf`,群点取 `othersOf`
    (只画别人的)、我那一枚追加在末尾并加亮边 —— 与卡片画布**同源同序**;票数不打折。
    ⚠ 带子只有 32px 高,坐标映射要**收进带内**(卡片画布 176px 高没有这个问题),否则上下沿会被切平。
