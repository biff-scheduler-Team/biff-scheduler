...
- **★ 票据明细 = 手填的「几张 / 座位 / 账号」(2026-09-24,`PLAN-20260924141442`)**:`biff.ticketinfo.v1` =
  `Record<场次 code, {count?, seats?, accountId?}>`,与 `biff.tickets.v1` **并列独立**:同以 code 为键、
  同处 `rebuildIndex()` 的 prune(共用 `tickets.ts::staleTicketCodes`),但**不是**同一条记录的两个半边。
  · 为什么另起一只键:三态那份回答「抢到了没有」(驱动「实际行程」筛选与同场人数),这份回答「手上几张、坐哪、
    票在谁的账号里」——塞进同一个值里会同时踩「`biff.*` key 只增不改」与「旧版 `normalizeTicketRecord`
    静默丢字段」两条。
  · **非法 = 没填,不是兜底成 1**:`count` 只认 1..10 的整数(格子徽章只在**显式填过**时才出现);
    `seats` **保位对齐**(下标 = 第几张票),只收**尾部**空项 —— 丢掉中间的空项会把「第 3 张是 14」读成「第 2 张是 14」。
  · **「共 N 张票」的唯一口径** = `ticket-info.ts::totalTicketCount`(「已抢到」∪「有明细」,没填张数的按 1 张)——
    它与概览里那条按**场次**数的「实际 N 场」刻意并存(一场可能买 2 张)。
  · 编辑入口 = 日程表格子**右键** + 格子上常驻的票按钮(触摸设备没有右键,单入口等于手机上功能不存在)。
- **★ BIFF 票务账号只存本机(2026-09-24,`PLAN-20260924141442`)**:`iffday.workspace.ticketaccounts.v1` =
  `{accounts: [{id, label, username, password}], defaultId}`。**绝不能落 `biff.*`** ——
  `sync-data.ts::readWorkspace` 收的是全部 `biff.` 键并上传云端,`backup.ts::BACKUP_PREFIX` 把全部 `biff.` 键
  写进**可以随手发给朋友的导出备份**;落那儿 = 明文密码上云 + 进备份文件。`iffday.workspace.*` 是既有本地专属
  命名空间(`account-sync.ts` 的 OWNER / cache / import 同处),上面两条通道都只认 `biff.` 前缀。
  · 明文存储是用户 2026-09-24 拍板的取舍;`password` 留空 = 不保存密码。读取路径**不写盘**(只有「保存设置」才落)。
  · 账号解析 `ticket-accounts.ts::resolveAccountOf`:单场 `accountId` → 默认账号 → `null`;
    覆盖的 id 在本机不存在(换设备后账号表不会跟着同步)时**回落默认账号**,而不是报错。
  · `ticket.biff.kr` 是跨域站点,前端**无法自动填充**它的登录框 —— 只能「打开站点 + 复制凭据」,
    UI 文案不得写成「自动登录」。
