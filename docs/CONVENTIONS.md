...
- **★ 三个「出口」各司其职(2026-09-11)**:`.ics`(`ics.ts`)= 喂日历(机器读);分享文案(`share.ts`)= 按日程读的
  ...
  **不要再合并成一个「导出」函数** —— 排序口径与信息取舍本就不同。
  ...
  · **「抢票」页 = 行程的开票期视图**(2026-09-15):路由 `/rush`,导航插在「我的行程」与「建议」之间
    (`App.tsx` 的 `nav` + `main.tsx` 路由 + `fullPage` 正则);按批次分组复用 `ScreeningCard`
    (自带票务三态),倒计时 / 开票时间复用 `extras.ts`(与顶栏「距第 N 批开票」同源),
    **不另写第二套倒计时 / 票价 / 票种口径**。批次判据说明与场次数见 `docs/plans/PLAN-20260915234414.md`。
  · **★ 分享图片吃的是同一份 `ShareOptions`**(2026-09-16,`PLAN-20260915234414` 修订 2):
    ...
    ⚠ **几何:`poster.ts::rowHeight()` 是行高的唯一来源**(`posterHeight()` 与 `drawPoster()` 都读它)——
      ...
      备选行 `ALT_H`、批次节头 `BATCH_HEAD_H` 都必须算进总高。
