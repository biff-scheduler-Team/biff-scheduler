...
- **★ 三个「出口」各司其职(2026-09-11)**:`.ics`(`ics.ts`)= 喂日历(机器读);分享文案(`share.ts`)= 按日程读的
  ...
  **不要再合并成一个「导出」函数** —— 排序口径与信息取舍本就不同。
  ...
  · **★ 分享文案版式 = CODE 行首 + 三行缩进块 + 无 emoji + 无 Markdown**(2026-09-16,`PLAN-20260916002752`):
    ...
    **不用 emoji**:旧版的 🎬 / 📅 / 📍 / 📝 全去掉(各端渲染宽度不一,缩进对不齐);`↳` 是箭头符号不是 emoji,
    备选块保留它。**不用 Markdown**(微信不渲染,`**` / `|` 会原样印出来)。
    状态列 = 影院短名 · 顺位标记 · GV 标记(` · ` 连接,空位自动省略)。
    GV 标记 = `映后`(含谈) / `仅正片`(弃谈) / `GV`(有 GV 但没配谈段时长)——
    ⚠ 它被分享文案 / 分享图片 chip / 方案「查看场次」弹层**三处共用**,改文案会一起变。
    日期分节头 = `【OCT 7 · 周三】`(场次数由概要行的「共 N 场」给出,不再重复印)。
  · **★ 分享文案的两个可选扩展(`share.ts::ShareOptions`,导出弹层的两个勾选框)**:2026-09-15 加。
    · `ranking` → 状态列多一个**顺位标记**(组内第 1 = `主选`,第 2 起 = `备选②③…`,带圈数字),
      同冲突组其余场次另起一块(前缀 `    ↳ `,同款三行缩进);
      **顺位 / 备选一律取「当前行程」的冲突组**(`plans.ts::groupMatesOf` + `plans.rankOf`),
      **不是方案快照** —— 方案每组只留第 1 顺位,拿它印顺位恒等于「主选」,朋友没法按顺位分工。
      备选块**省日期**(冲突按日期隔离 ⇒ 同组必然同日)、**必须印片名**(备选常是另一部片)。
    · `batching` → 先按开票批次分节(`【第 1 批 · 9/17 14:00 KST / 北京 13:00】`),节内再按日期分节;
      节头文案 = `extras.ts::batchHeading`(与「抢票」页横幅同源),空批次不输出。
      ⚠ **备选按**它自己的批次**归节**(2026-09-16,`PLAN-20260916011511`,唯一来源 `share.ts::shareUnits`):
      批次是**场次级**属性,主选与备选常常不同批(主选 349 第 2 批 / 备选 806 Actors' House 第 1 批),
      而备选若永远紧跟主选,第 1 批开票那天按节扫清单就**扫不到**它 —— 抢票当天唯一要用的信息被埋了。
      故与主选**不同批次**的备选抬成独立块(行首 `↳ `、顺位印 `349 的备选②`),归自己的批次节;
      **同批次**的备选仍紧跟主选(不抬)。概要「共 N 场」与「N 场」**只数主选行**(备选不是要看的那一场)。
  · **★ 导出范围默认「当前行程」**(2026-09-16,`PLAN-20260916135942`):`ExportDialog` 的下拉从「导出方案」
    扩为 **`导出范围`**,第一项「当前行程」(伪 id `ExportDialog.tsx::SCOPE_CURRENT`,概要走 `codesOutline`)
    **默认选中**;它 = `agenda-model.ts::topPlanCodes(plans)`(当前行程「每组第一顺位 + 共同场次」)⇒
    勾上顺位 + 批次后分享集合 = 行程全部场次(主选 + 备选)= **`/rush` 的两节逐条一致**。
    已保存方案仍是快照(`plan.codes`),可显式选、行为与改动前一致;选中的方案被别的标签页删掉 → 回落最后一个方案,
    都没有 → 回到当前行程(不再有「先保存一个方案」的死胡同)。`.ics` 与分享文案 / 分享图片同用一个 `rows`,
    同一范围下三者集合一致。
    ⚠ **场次取自所选范围,顺位 / 备选 / 批次一律取当前行程**(`plans.rankOf` + `groupMatesOf(plans.groups)` +
    `batch.ts::ticketBatchOf`)—— 两者混用会产出自相矛盾的清单:方案里的 `222` 印成主选、而当前组内第一顺位 `805`
    反倒印成「222 的主选」(2026-09-16 实测的线上症状)。
  · **★ 开票批次 = 场次级,判据唯一来源 `batch.ts::ticketBatchOf`**(2026-09-15,`PLAN-20260915234414`):
    第 1 批(9/17 14:00 KST)= 开闭幕 / Open Cinema / Midnight Passion / Actors' House / Community BIFF;
    第 2 批(9/21 14:00 KST)= 一般放映 / Master Class / Cine Class。逐场判据(优先级递降,穷举互斥):
    ① `tags` 含 `opening`/`closing` ② `tags` 含 `midnight` ③ `venue_id ∈ {bt}`(**Open Cinema =
    露天放映,按场地判** —— 官方定义就是 "Open Cinema(outdoor screenings)",官方公布的 8 部露天片单
    与 `bt` 20:00 的 8 场逐部吻合) ④ `code` 命中 `^9\d\d$`(**Community BIFF** = 官方编号 901–942,
    共 42 场,全在 MEGABOX `m1`–`m4`) ⑤ 活动 `kind = actors_house` ⑥ 其余第 2 批。
    ⚠ **别按影片 `unit === "Open Cinema"` 判**:同一部片既有露天场(第 1 批)也有室内重映(第 2 批),
    按单元判会漏 3 场露天场又多算 4 场室内场。⚠ 批次是**场次级**属性,不认影片。
  · **「抢票」页 = 行程的开票期视图**(2026-09-15):路由 `/rush`,导航插在「我的行程」与「建议」之间
    (`App.tsx` 的 `nav` + `main.tsx` 路由 + `fullPage` 正则);倒计时 / 开票时间复用 `extras.ts`
    (与顶栏「距第 N 批开票」同源),**不另写第二套倒计时 / 票价 / 票种口径**。
    批次判据说明与场次数见 `docs/plans/PLAN-20260915234414.md`。
  · **★ 「抢票」页用**一行一场的清单**,不复用行程页的 `ScreeningCard`**(2026-09-16,`PLAN-20260916005951`):
    用户原话「抢票和行程的 UI 重复太多了 … 把需要写的 code 和顺位写一写就行了」——
    卡片那套(海报 / 豆瓣 / 时长 / 票价 / 徽章 / GV 控件 / 冲突说明 / 资料·定位·加入行程·讨论 / 同场人数)
    是「挑片 / 出门找地方」的语言,抢票当天全是噪声。清单行 = `CODE · 起止时间(有效结束) · 日期 · 片名 · 影院短名 · 顺位`,
    行下只留**票务三态**(复用 `ScreeningTickets.tsx::ScreeningTicketControl` —— 它是「已抢到 N 场」与
    行程页「仅看实际行程」的唯一数据入口,**不许在本页另写一份或删掉**)。
    · 顺位**只取** `plans.ts::buildPlanSet` 的 `rankOf`(行程页冲突组内拖出来的次序),本页不另算;
      文案取 `share.ts::rankMark`(`主选` / `备选②`,与分享文案 / 分享图片同一份措辞);
    · 日期取 `util.ts::dateInfo`(`OCT 8`,全站唯一日期写法);样式落在 `pages/rush.css`
      —— **不追加进 `style.css` / `agenda-parity.css`**(命中 TEST-MAP `allOn`,会触发三浏览器全量 E2E);
    · ⚠ 行程页**仍是**顺位的唯一写点:抢票页只读不拖,别在两个页面各做一套排序交互。
  · **★ 分享图片吃的是同一份 `ShareOptions`**(2026-09-16,`PLAN-20260915234414` 修订 2):
    ...
    · `ranking` → 行内状态列多一个顺位标记(取 `share.ts::rankMark`,与文案**同一份措辞**);
      该场下方画备选行(`↳ 备选②  CODE  时间  片名  影院`,标记取 `share.ts::ALT_MARK`)、
      时间 / 片名 / 影院 / 顺位的取值口径与文案的备选块**逐项一致**;
    · `batching` → 图按开票批次分节,节头文案 = `extras.ts::batchHeading`(与文案 / 抢票页同源,绘制走 `drawBatchHead`);
      ⚠ 图上**备选同样按自己的批次归节**(与文案共用 `share.ts::shareUnits`):与主选不同批次的备选
      画成 `↳ 349 的备选②  CODE  时间  片名  影院` 的**独立一行**(`PosterRow.altOf`,不画海报卡片 ——
      画成卡片会把它抬到与主选同等的视觉重量),且**不计入** `PosterDay.count`。
    · 海报行首改为 **`CODE  时间`**(与文案同序:抢票时按 CODE 找场次),影院行不再重复印 CODE。
    ⚠ **几何:`poster.ts::rowHeight()` 是行高的唯一来源**(`posterHeight()` 与 `drawPoster()` 都读它)——
      各算一份会出现「按 3000 高建画布、实际画了 3200」,**多出来的内容被裁掉且不报错**(最难查的一类);
      备选行 `ALT_H`、脱离主选的备选行 `ALT_ALONE_H`、批次节头 `BATCH_HEAD_H` 都必须算进总高。
    ⚠ **模型结构:`PosterModel.sections` 是唯一结构,`days` 是它的扁平派生视图** ——
      别单独再算一份,否则会出现「图上分了节、列表(缩略图 / 方案「查看场次」弹层)里没有」。
