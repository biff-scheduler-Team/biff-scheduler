...
> 最后更新:2026-09-24(**我的行程 → 日程表上的座位表**(用户「我的行程中加一个显示自己有几张票的组件」→
> 「日程表上可以右键编辑 然后显示在日程表上 能填上座位号…」→ 验收后「直接通过操作去添加座位表吧…
> 用添加的座位数作为票数」)—— 新增并列键 `biff.ticketinfo.v2`(座位行;**行数即票数**,与票务三态同一
> prune 点,含 v1 一次性迁移);日程表格子右键 / 票按钮即改 + 张数徽章 + 概览新增「共 N 张票」
> (与按**场次**数的「实际 N 场」并存)。**账号 / 密码方案已整体撤销**:`ticket.biff.kr` 会 302 到第三方
> 平台 `biff.maketicket.co.kr`,订单页在**另一个域**的登录态后面 —— 跨域写不进登录框、建不了会话,
> 存密码收益≈0 而代价是明文落盘,故不留任何键、任何字段。见 `PLAN-20260924141442` 修订 3);
> 同日**修订 5**(用户「刚才你加的座位系统姓名 预约号 接入导入系统 然后把我的票生成一个 json」):
> 票据明细升到 `biff.ticketinfo.v3`(加 `name` / `bookingNo` / `account` 三个**每场一条**的可选文本字段,
> 故 v2→v3 无需单独迁移函数),新增**票务 JSON 导入**通道(`ticket-import.ts`;只写票务的**增量**通道,
> 与备份的「整体替换」区分开,并连带并进空行程 + 标「已抢到」);
> ⚠ 顺手修掉一个真 bug:座号上限 12 → 32(BIFF 真实座号 `Floor 1 · R8 S23` 有 15 字符,旧上限会**静默截断**);
> `account` 只存账号**名**,不是密码);
> 同日**修订 4**(用户「有不划位的 需要考虑这种情况」):票务弹层新增「这场不划位（自由入座）」档 ——
> 勾上后只记张数、不填座位号;**该档位不落库**(落库仍是 N 个空座行,重开按「全空」判回,故不需要新 key / 迁移);
> 初始档位由露天场(`bt`)给,但**排除开闭幕**(`001` / `002` 同在 `bt`,那是固定座席的典礼场);
> 座位号全空时 tooltip 改说「N 张，座位未填」;代价是数据上分不清「不划位」与「座位号还没抄」,用户选择不加字段);
> 前次更新:2026-09-23(**徽标对比度达标**(用户「开始实现批一」;`ui-ux-pro-max` Skill 审核 → `PLAN-20260923122409`)——
...
> 我那一枚是只读 `.rb-dot`)—— 于是弹层里也认得出自己那枚、位置与卡片逐字一致;「拖出张贴区」有了
> **明显提示**(浮标换装 + 「松开 · 收回暂存区」标签 + 源片暂存区点亮),顺带把落点判据从「卡片」收敛到
> 「画布」(`ownCanvasAt`),修掉「卡片亮着灯、松手却收回」的假承诺;「更多」菜单里的
> 「导出与分享」不再逐字折行(埋点从 DOM 锚点移到 `onAction` 里显式上报)。
> 见 `PLAN-20260923101147` / `PLAN-20260923103830` / `PLAN-20260923104622` / `PLAN-20260923110401`);
> 前次更新:2026-09-22(**红黑榜「生成分享图」**(用户:「请你给这个我贴过的页面做一个分享按钮 然后弄成分享图片
> 的形式 黑榜红榜默认 TOP10 我贴过的不限场数样式 弄好看点吧 底部署名网站 by gaaiyeoi 和 cit忘记怎么拼了
> 你找找就行」)—— 控制行新增「生成分享图」→ 弹层预览 → 下载 PNG / 复制图片;图里 **总数 / 红 / 黑三榜各取
> TOP10**(逐字复用页面三档排序)+ **我贴过的全部(不限条数)**,每行**两列**:左文字、右一块
> **专门的贴纸区**(与卡片画布同版式;落点与组成复用 `spotOf` / `tiltOf` / `othersOf`,我那一枚加亮边);
> **出图前可勾选要分享哪几节**(总数 / 红 / 黑 / 我贴过的,默认全选,一节都不勾则停用下载),并**当场报出这张图的
> 尺寸与体积**(如 1080 × 10392 px · 1× · PNG 约 3.0 MB;高度只由行数决定);署名**头部与底部各一处** + 底部署名 `biff.lcandy.co` /
> `by @gaaiyeoi 和 by @lcandy2`(**修订 1**:署名写 GitHub **账号名**,不是提交者显示名 `citron` —— 对齐旧版
> 页脚;`App.tsx` 页脚同步改)。**顺带修掉一个真 bug**:分享图原来看不住画布**面积**上限(iOS Safari 约 16.7M px),
> 2× 下逻辑高超过 ~3700 就 `toBlob` 静默出空图 —— 长行程的分享图在 iPhone 上一直是一张空图;
> 新增 `posterScale()`,宁可 1× 也不能出空图。顺带把两张分享图的**笔刷与配色抽成共用模块**(`poster-brush.ts`)、
> 贴纸形状抽成 `sticker-shape.ts`、复制失败退化下载抽成 `share-image.ts`。
> 见 `PLAN-20260922164728`);
> 前次更新:2026-09-22(**红黑榜贴纸渲染改造**(用户:「我也觉得会有性能问题 但是我又想在在一个电影中显示
> 所有贴纸 有什么好的方案吗」)——「每卡最多 16 枚」的上限与「+N」角标一并取消,**票数几枚就画几枚**;
> 渲染预算的单位从「每部片画几枚」换成「**同时画几张卡**」:屏幕外的卡一枚不画(视口边距 1200px)、
> 视口内全量,只读贴纸改由**每卡一张 `<canvas>`** 绘制(DOM 里只留我贴的那一枚 —— 它要能拖),
> 配按值的**重绘守护**(参数没变不重画)与**跨屏 / 缩放 DPR** 适配;另加「**放大看全部**」弹层(复用 S2 `Dialog`)。
> 本机压测 299 部 / 20629 票:DOM 节点 8571 → 4955、贴纸 DOM 4008 → 60、滚动时画布只占 1% 的时间。
> 顺带修掉一个只有放开上限之后才看得见的真 bug:`spotOf` 的纵坐标会挤成一条横带(补哈希雪崩)。
> 见 `PLAN-20260922145815`);
> 前次更新:2026-09-22(**红黑榜四项修整**(用户三问:「别人贴的 和我自己贴的 贴纸长得不一样?」
> 「上面的统计数据应该显示全局的 然后自己的应该有什么其他的显示逻辑?」「现在最多只显示 16 枚吗? 为啥会这样」)——
> ① 别人的贴纸与我贴的**完全拉平**(同 26px、同实心度),只剩 `pointer-events:none` 的「能不能拖」可区分;
> ② hero 统计改成**全站为主**(大字总票数 + 红 / 黑拆分)+ 我的一行小字(标记 / 已贴 / 还能贴)——
> 改版前那三格**全是个人进度**,全站票数算出来却只用于判断「榜空不空」,一个字都没显示;
> ③ 新增「只看我贴过」筛选(独立参数 `only=mine`,不挤进 `sort`,避免破坏顺序冻结机制);
> ④ 修 `crowdStickers` 把**少数派颜色四舍五入抹掉**的真 bug(1 红 / 100 黑 → 画布上一个红点都没有,
> 而卡片写着「红 1」),并给画不下的票数加「+N」角标(16 枚不再被读成真实票数)。
> 贴纸颜色另收敛为 `--rb-red` / `--rb-black`(原先 6 处硬编码)。见 `PLAN-20260922142903`);
> 前次更新:2026-09-22(**修「更多」菜单里「导出与分享」逐字折行**(用户:「导出与分享 变成每个字一行了」)——
> S2 的 `MenuItem` 只把**字符串** children 包成 `slot="label"` 的文本盒,元素 children 会掉进菜单 grid
> 第一列那段 9px 留白里(实测 12px 宽 × 105px 高,每个字一行);文字改回字符串,该项的 `click=export`
> 埋点从 DOM 锚点移进 `openHeaderAction` 里显式上报(`onAction` 鼠标与键盘都会走到)。见 `PLAN-20260922142922`);
> 前次更新:2026-09-22(**「我的行程」第三轮**(用户两条:①「不需要显示 冲突组顺位这个组件 直接铺满」;
> ②「点击取消需要二次确认」)——上一轮那**两栏版式整体撤销**:右侧栏(冲突组顺位 + 场次详情)与画布上方的
> 「顺位撞车」提示一并下线,日程表画布回到**单列铺满**;画布上点掉一场(原先静默、无撤销)改走**站内确认弹层**,
> 「只有一场会连带移除选片」的判据抽成 `screening-actions.ts::removalDropsPick()` 供弹层与 picker 共用。
> ⚠ **顺位机制一字未动** —— 顺位卡现在只在「卡片」视图里按日就地展开。见 `PLAN-20260922123138`);
> 前次更新:2026-09-22(**「我的行程」第二轮 + 方案整体下线**(用户五条:①顶部辅助区挤满;②首屏被离散层级下挤、
> 时间表被挤到左下角;③单厅时右侧留大片空白;④方案管理埋在最底部;⑤圆角与灰阶混杂、提示文字对比度低)——
> **「已保存方案」整体下线**(含导出范围里的方案选项,`biff.savedplans.v1` 留废键不迁移);
> 顶栏三项收进「更多」菜单、售票提示移出 top bar 成独立通知条;**日期条并进唯一一条工具栏**、
> 概览收进页头、画布右侧改放**两栏侧栏**(当天顺位卡 + hover 场次详情,可折叠);
> 新增三档圆角 token 与 `--muted-strong`。见 `PLAN-20260922105228`);
> 前次更新:2026-09-22(**「我的行程」信息架构收拾**(用户四条:①「保存当前方案」悬在日程表与已保存方案之间的空白里;
> ② 工具栏与画布图例分成两行、功能混杂;③ 日期条在工具栏下方;④ 方案卡的「删除」离方案名太远、缺二次确认)——
> 保存按钮提到顶部工具栏右侧;两条控件行并成**一条**(左:图例 / 添加转票场次 / 仅看实际行程,右:日程表-卡片 / 缩放 / 保存);
> 日期条上移到概览条下方;**方案卡的删除钉到方案名同一行、加二次确认**。见 `PLAN-20260922103307`);
> 前次更新:2026-09-22(**「抢票」整体下线 + 分享去掉顺位 / 批次 + 主导航重排**(用户:「抢票入口也去掉吧」+
> 「分享的地方去掉顺位和批次了 排片表 影片库 我的选片 我的行程红黑榜 吃喝数据分析建议 顶部顺序改成这个」)——
> 导航项、`/rush` 路由、`RushPage.tsx`、`batch.ts` 全删,`rush.spec.ts` 迁为 `share-export.spec.ts`;分享文案 / 图片里的
> 顺位标记、备选行、开票批次节头与导出弹层那两个勾选框一并去掉(`ShareOptions` 整套退役,**选片顺位本身保留**);
> 主导航改为 排片表 / 影片库 / 我的选片 / 我的行程 / 红黑榜 / 吃喝 / 数据分析 / 建议。见 `PLAN-20260922102751`);
> 前次更新:2026-09-22(**「讨论区」整体下线**(用户:「去掉讨论区入口 相关组件也去掉」)—— 导航项、`/discussions` 页、
> 场次讨论弹层、前端接口封装、讨论相关 CSS 与三个 E2E 用例全删;「数据分析 / 抢票」里的「场次讨论 N」指标
> 与读数链路(`screening-counts` 的 `discussions` → `rush-crowd.ts::talkBoard`)一并清掉。**只动前端**:后端接口 /
> 契约 / D1 表原样保留。见 `PLAN-20260922101227`);
> 前次更新:2026-09-22(**发版后顶部提示「有新版本」并一键刷新**(用户:「现在发版一定要用户 ctrl shift R
> 才能看到新版本吗」)—— 根因不在 HTTP 缓存(首页本就是 `max-age=0, must-revalidate`),而在 workbox 把
> `index.html` 带 revision 预缓存了,导航请求直接吃缓存,**那一层绕过所有 HTML 缓存头**;线上 `registerSW.js`
> 又只注册、没有「新版接管后刷新」。现在在 `main.tsx` 顶层挂 `controllerchange` 判定(带「首次安装的 claim
> 不算新版」的防误报前提)+ 切回前台/30 分钟定时 `registration.update()` 兜住长开的标签页,检测到就在页面
> 最顶部出一条 sticky 提示,点「刷新」才 reload —— **不做静默强刷**。见 `PLAN-20260922100704`);
> 前次更新:2026-09-21(**「我的行程」新增「日程表」视图**(用户:「把我的行程也重构日程表显示吧 只显示我的行程 而且
> X 轴只显示我有影片的影院 感觉比较好看一点」)—— 与排片表**共用** `components/ScheduleGantt.tsx`(X 轴 = 影厅列、
> Y 轴 = KST 时间),画布只画我的场次、X 轴只列我有影片的影厅、时间轴跟着我的场次收;顶部日期条切单日,冲突在画布上
> **画连线**、顺位卡留在当天画布下方;页头加「日程表 / 卡片」两档(默认日程表,旧卡片列表是回退路径)。
> 同轮修订 1:行程卡片去掉「定位」与「讨论 N」入口(用户:「既然直接是日程表了 可以不需要定位 正好也把讨论区去掉吧」)
> —— 「定位当日」保留、`/discussions` 页面与导航项保留。见 `PLAN-20260921223658`);
> 前次更新:2026-09-20(**类目名太长时可 hover 看全名**(用户:「Y 轴名字太长会被遮挡 然后能不能 hover 展示全名呢」)
> —— 两个横排图的类目轴加 `tooltip: { show: true }`(ECharts 的 axis tooltip):轴上仍按 `width: 130` 截断省地方
> (面板只有 ~460px,加宽标签就是拿柱子换文字),hover 标签弹出**完整**片名,弹出的浮层走同一套
> `sharedTooltip` token(实测底色 = `--surface`,不是 ECharts 默认黑底)。见 `PLAN-20260920202312`);
> 前次更新:2026-09-20(**明细表扩到 30 行、一次看得见 10 行**(用户:「影片明细 · 想看 TOP 10 改成 TOP 30 然后只显示 10 个 剩余的用滚动条实现展示」)
> —— 表格最多取 30 行,包进 `.ra-table-scroll`(`max-height: 354px` = 表头 23.5 + 10 行 × 33,2026-09-20 实测值),
> 超出部分滚动查看;`thead th` 做 sticky(滚到第 20 行仍知道哪列是哪列);标题与「按场次看」开关移到
> 滚动区**外面**(滚动时仍能切视角)。场次视角同步(同一张表)。见 `PLAN-20260920201606`);
> 前次更新:2026-09-20(**修「hover 柱子被高亮块遮挡」**(用户:「hover 柱子的时候 会出现柱子直接被遮挡 是不是柱子背景色的原因」)
> —— 根因是 ECharts 6 的 `axisPointer` 默认 `z: 50`(压在 bar series 的 z=2 之上),而我们又把它的
> `shadowStyle.color` 写成不透明的 `--raised`,于是 hover 时整行 / 列被一块实色盖死、柱子看不见。
> 修法:`z: 1`(退到网格线与柱子之间)+ 从 `--line` 派生半透明(`withAlpha`,**只派生不新增色源**)。
> E2E 新增一条断言钉住「高亮块排在柱子之前 + 带 alpha」;两个方向都验过红(见 `PLAN-20260920194321`);
> 前次更新:2026-09-20(**明细表加「按场次看」开关**(用户:「影片明细 · 想看 TOP 10…需要勾选一个选项 能够区分影厅 知道哪一个场次最多人」)
> —— 复选框「按场次看(区分影厅)」:勾上后**同一张表**切成场次视角(影片 / 场次 / 时间 / **影厅** / 抢票人数),
> 按**单场人数降序**取前 10 —— 第一行就是人最多的那一场;默认不勾时与改动前完全一致。
> ⚠ 影片视角的「抢票人数」是各场次数**之和**、场次视角是**单场**人数,两个数各算一份、互不顶替。
> 见 `PLAN-20260920193834`);
> 前次更新:2026-09-20(**「群体行为与口碑」的两个下拉换成搜索框**(用户:「群体行为与口碑 中的场次电影全部采用搜索框」)
> —— 「电影」「场次」改成输入即筛(片名 / 场次号 / 日期 / 影院);原来的「关键词」框**删除**(与「电影」框重复);
> 选中语义改由搜索框承担:**筛到唯一一部片 / 唯一一场就算选中**(口碑构成环与「本场同场」照常出现)。
> ⚠ 代价:原先「选片后场次下拉只列该片场次」这条联动没有了(场次框不再是列表)。见 `PLAN-20260920193638`);
> 前次更新:2026-09-20(**图的说明收进右上角 ⓘ**(用户:「太多解释性文案了 全部放到图的右上角的 tooltip 里面吧」)
> —— 每张图下面那行口径小字改成画布右上角一个 `<details>` ⓘ(点击 / 键盘都可达),点开才显示;
> 红黑票没有票时给**可见**的空态文案(状态提示不该藏进 ⓘ);E2E 新增「默认不可见、点开才可见」一条。
> 见 `PLAN-20260920193412`);
> 前次更新:2026-09-20(**「抢票分析」页更名为「数据分析」**(用户:「抢票分析 不要叫这个名字 叫数据分析吧」)
> —— 导航项 / `<h1>` / 区域 `aria-label` 改成「数据分析」;路由 `/rush-analysis` 与模块代号**不动**
> (书签 / PWA 缓存 / TEST-MAP 的契约,改名只落在显示层);E2E 新增一条断言钉住「导航名 === 页面标题」。
> 见 `PLAN-20260920193032`);
> 前次更新:2026-09-20(**分析页铺满 + 重新排版**(用户:「为什么卡片不会填满?」)—— 根因不是卡片,
> 而是**页面元素作为 `.workspace` 的 flex 子项没给宽度**(`flex-basis:auto` 取内容宽 → 只铺 620px);
> 修法照既有三个整页写进 `style.css:1489` 那份名单(故本次按 allOn **应跑三浏览器全量 E2E,尚未跑**)。
> 同时:影片明细 24 行 → **TOP 10**;数字条 / 筛选条改 grid 等分(原来挤在左边、右半边空);
> 红黑票图改**横堆叠**(竖放的 10 个中文片名会被抽稀成只剩一条);环形图图例移到底部(原右侧竖排**压在扇区上**);
> 年份榜 28 个 → 影片数最多的前 8;红票/黑票改用贴纸本义色(原先「红票」被画成语义绿)。
> 另修一个静默 bug:按需注册漏了 `TitleComponent`,环形图圆心合计**一直没画出来**且不报错。
> 见 `PLAN-20260920203010` 修订 5);
> 前次更新:2026-09-20(**抢票分析页精简为两组 + 图表换 ECharts**(用户:「太复杂了 而且前端不好看」)
> —— 页面只剩「群体行为与口碑」(可按**电影 / 场次**筛选想看人数)与「我的观影画像 / 影片分析」
> (国家 / 单元 / 评分分档 / 年份 / 时长);难度榜、需求分布、集中度、抢票结果、口碑散点、供给、
> 行动建议、使用热度**与它们的模块和单测一并删除**(票务结果与事件流水的**采集保留**,但已无展示面)。
> 图表层从 recharts 换成 **ECharts 6**,视觉做法照 ma-agent-harness 的
> `frontend/src/components/workspace/analytics/chart-design-tokens.ts`(运行时读令牌 + 订阅主题 +
> 统一 tooltip + 圆角柱 + 环形图);`charts` chunk 97,317 → 193,261 B gzip,首屏只多 741 B。
> 见 `PLAN-20260920203010` 修订 4);
> 前次更新:2026-09-20(**分析页去文字化**(用户反馈「太多字了,尽量都图表化」)—— 先探针量出
> 全页 7237 字 / 70485 px,其中**难度榜一个分组占 3068 字 + 795 行 + 64208 px**(每行还抄一句
> 人话判据)。改法不是删信息而是**换表达**:难度榜默认只列前 20 行(可展开、搜索不受限)、每行加
> 「难度条」并把判据移进 `title`;批次卡→堆叠条、制产国→横排图、「有争议」→一行 chip、讨论榜→图、
> 行动建议新增「要盯的场次」图;全部口径说明压到一句话、完整解释移进 `title`。
> 结果:**7237 → 5433 字(−25%),70485 → 7583 px(−89%)**,图表 19 → 21。见 `PLAN-20260920203010` 修订 3);
> 前次更新:2026-09-20(**抢票分析第 2 轮:四组新维度** —— 新增「供给与排片结构 / 我的观影画像 /
> 群体行为与口碑 / 行动建议」四个分组,全部并入 `/rush-analysis`(共七个分组)。新增 4 个纯逻辑模块
> (`rush-supply` / `rush-portrait` / `rush-crowd` / `rush-advice`,+42 条单测)与 4 个分组组件,
> 以及一个复用型条形图组件 `components/charts/bars.tsx`(第 1 轮五个图各写一个,本轮起「同形状换数据」
> 不再复制实现)。主包 gzip 81,723 → 81,774 B(+51),增量全落在懒加载页 chunk;
> E2E 8 → 13 条。见 `PLAN-20260920203010` 修订 1(第 3 轮事件流水待做));
> 前次更新:2026-09-20(**分析页图表化 + 动作按钮收敛** —— 抢票分析页五个区块改用 recharts
> (难度散点 / 需求趋势与时段 / 帕累托 / 结果堆叠条 / 口碑四象限),配色从既有 token 运行时读取并订阅
> `data-theme` 重绘;recharts 实测 123.5 KB gzip,故拆独立 chunk + **分析页路由级懒加载**,
> 主包 gzip 反而 84,778 → 81,723 B。同时把散在五处的「加入我的选片 / 加入行程 / 移除影片」收敛成
> 「想看 / 取消想看 / 排进行程 / 移出行程」,唯一来源 `apps/web/src/actions-copy.ts` 并加机械守卫单测。
> 见 `PLAN-20260920203010`（三轮规划第 1 轮:第 2 轮四组新维度、第 3 轮事件流水待做）);
> 前次更新:2026-09-20(**新增「抢票分析」页** —— 需求 / 难度 / 结果 / 口碑四个面量化全站票务:
