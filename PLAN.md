...
> 最后更新:2026-09-24(**我的行程 → 日程表上的票务标注**(用户「我的行程中加一个显示自己有几张票的组件」→
> 「日程表上可以右键编辑 然后显示在日程表上 能填上座位号 在设置中配置 biff 的官方网站的账号密码 能提供
> 快速跳转对应的票务 然后每一张票也能单独设置账号」)—— 新增并列键 `biff.ticketinfo.v1`(张数 / 座位 /
> 单场账号覆盖;与票务三态同一 prune 点、零迁移)与**本地专属**的 `iffday.workspace.ticketaccounts.v1`
> (账号 + 明文密码:**刻意不落 `biff.*`** —— 落那儿会被账号云同步上传、并被写进可分享的导出备份);
> 日程表格子右键即改 + 张数徽章 + 概览新增「共 N 张票」(与按**场次**数的「实际 N 场」并存);
> 设置新增「BIFF 票务账号」区。见 `PLAN-20260924141442`);
