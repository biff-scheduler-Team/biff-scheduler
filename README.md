...
影片库、我的选片和我的行程均可通过顶部导航打开独立页面。排片表右下角「我的观影」按钮打开快速查看面板，可在选片与行程之间切换，也可打开对应完整页面；面板不压缩表格。手机使用底部面板。选中场次不会自动打开面板，定位场次时会收起面板。桌面和手机共用时间纵轴、影厅横轴的排片表，电影时长对应卡片高度。排片表内部只横向滚动，纵向随整页滚动，影厅表头随页面固定。顶栏的「回到旧版」打开 `/legacy/`，保留原版完整界面；旧版顶栏可返回新版。
...
排片表可按字幕、影厅和 GV 筛选。影片库与排片表使用两套独立筛选，都会记住上次选择。影厅支持包含与排除模式，以及按影院和区域选择。时间轴支持缩放、横向拖动、整点筛选和场次定位。缩放保留页面当前时间位置，「适应」按可见宽度排列完整影厅列。
