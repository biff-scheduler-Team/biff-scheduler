...
test("wheel zoom preserves page time and has no internal vertical scrolling", async ({ page }) => {
  ...
  const before = await timeAtCenter(page, 1);
  ...
  expect(Math.abs(await timeAtCenter(page, .9) - before)).toBeLessThan(1);
...
test("external zoom settings preserve the time under the page center", async ({page, context}) => {
  ...
  const before = await timeAtCenter(page, 1);
  await other.evaluate(() => localStorage.setItem("biff.settings.v1", JSON.stringify({...JSON.parse(localStorage.getItem("biff.settings.v1") ?? "{}"), zoom: .9})));
  await expect(page.getByLabel("缩放比例", {exact: true})).toHaveText("90%");
  expect(Math.abs(await timeAtCenter(page, .9) - before)).toBeLessThan(1);
  await other.close();
});
