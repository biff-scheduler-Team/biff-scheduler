...
test('info preview follows the mouse without picking or navigation; full details remain available', async ({page}) => {
...
test('hover preview stays open across the gap, mouse clicks stay non-sticky, keyboard Escape restores focus', async ({page}) => {
  await ready(page,'/schedule?date=2026-10-07');
  const trigger=page.getByRole('button',{name:'场次 008 影片资料',exact:true});
  await trigger.hover();
  const preview=page.getByRole('dialog',{name:'场次 008 影片预览',exact:true});
  ...
  // 回归(`PLAN-20260917003630`):鼠标点一下不再把弹层钉住 —— 指针移开即自动收起,
  // 原先点一下会切成 pinned,必须再点「关闭」/Escape 才收。
  await trigger.click();
  await expect(preview).toBeVisible();
  await page.mouse.move(0,0);
  await expect(preview).toHaveCount(0);
  // 键盘没有 hover 可依赖:pinned 仍然点开点关,Escape 关掉并把焦点还给触发按钮
  await trigger.focus();
