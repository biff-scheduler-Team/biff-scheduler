...
/* ---------- 已保存方案 —— 已整体下线(2026-09-22,`PLAN-20260922105228`) ----------
 * 用户原话:「我觉得现在方案没有什么意义了 去掉也可以」→ 选「整体下线」:
 *   `SavedPlan` / `savedPlans` / `savePlan` / `deletePlan` / `loadSavedPlans` /
 *   `persistSavedPlans` / `sameCodeSet` / `nextPlanName` / `planById` 全部删除,
 *   导出范围的「方案」选项、`PlanShowsDialog`、「已保存方案」区块也一并下线。
 *
 * ⚠ **`biff.savedplans.v1` 这个键故意留着不删**(数据契约只增不改,同 `biff.discussionprivacy.v1`):
 *   不再读、不再写、不做迁移。这样「加载旧数据 → 原样回写」的字节级断言
 *   (`compatibility.spec.ts`) 与 `backup.ts` 的 `biff.` 前缀快照都照常通过;
 *   哪天想重启这个功能,键里的旧方案还在。
