...
export interface Venue {
  ...
  code?: string;
  /**
   * 该厅座位数(「数据分析」页算竞争倍率的**分母**,2026-09-20)。
   *
...
export interface TicketInfo {
   ...
   *
   *  ★ **「不划位」没有独立字段**(2026-09-24 修订 4,用户「有不划位的 需要考虑这种情况」):
   *    自由入座的场次一律落成**全空**的座位行(`["", ""]`),「不划位」这个档位由弹层从
   *    「全空」**推断**出来(`ticket-info.ts::allSeatsBlank`),只活在界面上,**不落库**。
   *  ⚠ 已知代价:数据上分不清「不划位」与「划位但座位号还没抄」—— 两者票数完全相同,
   *    故 tooltip 只能说「N 张，座位未填」(两句都成立)。 */
  seats?: string[];
  /** 持票人姓名(如 `RAOJIARUI`)。**每场一条**,同场多张票共用。
   *  (2026-09-24 修订 5:用户给了 17 笔预约的明细,要把姓名 / 预约号也记下来。) */
  name?: string;
  /** 预约号(BIFF 订单号,如 `269LEYE2LGJ37124A`)—— 换实体票 / 查订单时要报的那串。**每场一条**。 */
  bookingNo?: string;
  /** **账号名**(如 `foxmail` / `418865265`)—— 记的是「这笔票在哪个 BIFF 账号下」,**每场一条**。
   *
   *  ⚠ 它**不是凭据**:修订 3 撤销的是「存账号密码」,这里只是一个纯文本标签
   *    (`PLAN-20260924141442` 修订 5)。**绝不许往里填密码** —— 本结构落在 `biff.` 前缀下,
   *    会随片单同步到账号云端。 */
  account?: string;
}
